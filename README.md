...
Most of this app's code was generated by **Claude (Anthropic)** under my direction. I did the on-device debugging and testing, drove the problem-solving (e.g. finding that the Honor "Recommendations" identifier lives in the notification's channel ID), and made all the design, product, and release decisions — but I did not hand-write most of the code.

The app is built and tested on **Honor** (Magic UI). It should work on other Huawei/Honor devices and Android 8.0+ generally, but behaviour on hardware I haven't tested may vary. Use at your own discretion; it's provided as-is under the GPLv3, with no warranty.
